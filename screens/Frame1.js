...
const Frame = ({ route }) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://192.168.8.100:3000/api/delete-entry/${id}`);
      fetchServiceData(); // Refresh maintenance data after deletion
    } catch (error) {
      console.error("Error deleting maintenance data:", error);
    }
  };
  const handleAddEntry = () => {
    navigation.navigate('IPhone1415ProMax1',{userId}); // Navigate to the "AddEntry" page
  };
      ...
      <ScrollView horizontal={true}>
        ...
        <View style={styles.table}>
          <View style={styles.tableRow}>
            ...
            <Text style={styles.headerCell}>Service Provider</Text>
